  ...
  public class Tracker
  {
    ...
    public void TrackOneAllyCode(PlayerSettings setting, AuthResponse auth)
    {
      try
      {
        ...
        Console.WriteLine(result.PlayerName + ": Arena : " + result .SquadArenaRank + " Fleet : " + result.FleetArenaRank);
        ...
        if (!rank2.HasValue)
        {
          Console.WriteLine(string.Format("`{0}` is at {1}", (object) result.PlayerName, (object) rank1));
        }
        else
        {
          int? nullable1 = rank2;
          ...
          if (nullable2.GetValueOrDefault() > num2 & nullable2.HasValue)
              Console.WriteLine(string.Format("`{0}` climbed from  {1} to {2}", (object) result.PlayerName, (object) rank2, (object) rank1));
          else
              Console.WriteLine(string.Format("`{0}` dropped from {1} to {2}", (object) result.PlayerName, (object) rank2, (object) rank1));
        }
      }
      catch (Exception ex)
      {
